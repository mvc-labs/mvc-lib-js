...
Transaction.prototype.checkedSerialize = function (opts) {
  ...
  if (serializationError) {
    serializationError.message += ' - For more information please see: ' +
      'https://mvc.io/api/lib/transaction#serialization-checks'
...
Transaction.prototype.toBufferWriter = function (writer) {
  return this.version in [1,2] ? this.legacyTxid(writer) : this.newTxid(writer)
}

Transaction.prototype.legacyTxid = function (writer) {
...
Transaction.prototype.newTxid = function (writer) {
  writer.writeUInt32LE(this.version)
  writer.writeUInt32LE(this.nLockTime)
  writer.writeUInt64LEBN(BN.fromNumber(this.inputs.length))
  writer.writeUInt64LEBN(BN.fromNumber(this.outputs.length))
 ...
 * and a slightly fancier format recognized by mvc:
 *
 * ```
 * {
 ...
 * }
 * ```
 * Where `address` can be either a string or a mvc Address object. The
 * same is true for `script`, which can be a string or a mvc Script.
 *
